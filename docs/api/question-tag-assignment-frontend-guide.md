...
| `422` | AIが有効なタグを選べない | モーダルを保持し、手動選択を促す |
| `429` | Geminiのレート制限 | モーダルを保持し、少し待ってからAI再試行または手動選択を促す |
| `502` | Gemini通信失敗 | モーダルを保持し、AI再試行または手動選択を促す |
| `503` | Geminiが利用不可 | モーダルを保持し、AI再試行または手動選択を促す |
| `504` | Geminiタイムアウト | モーダルを保持し、AI再試行または手動選択を促す |
...
- AI再試行失敗の `422` / `429` / `502` / `503` / `504` は再試行可能です。ただし自動ループはせず、ユーザー操作で再実行してください。`429` のときは連続再試行を避けてください。
