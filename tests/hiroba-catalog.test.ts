...
describe('fixed hiroba catalog', () => {
  it('contains 27 unique fixed hirobas grouped into sections', () => {
    expect(HIROBA_CATALOG).toHaveLength(27)
    expect(new Set(HIROBA_CATALOG.map((hiroba) => hiroba.id)).size).toBe(27)
    expect(new Set(HIROBA_CATALOG.map((hiroba) => hiroba.slug)).size).toBe(27)
