---
name: setup
description: yoriai のプロジェクト構築・MCPサーバー導入スキル。ゼロからのプロジェクト再現手順、Claude Code用MCPサーバー(hono-docs, context7, next-devtools, shadcn)の導入コマンド、tsconfigパス設定をまとめる。新規環境構築や新しいMCPサーバー追加時に使う。
...
### 共通の注意点
- いずれもリモート/ローカル問わず初回利用時に承認が必要（`claude` 起動時 or `/mcp` で承認）
- `claude mcp list` で登録状況・承認状況を確認できる
- 削除は `claude mcp remove <name> --scope project`
