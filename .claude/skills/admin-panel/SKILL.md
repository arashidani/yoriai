...
- タグのカテゴリーは自由入力ではなく`GET /api/admin/tag-categories`を使う単一選択ドロップダウンにする。使用中カテゴリーの削除は外部キー制約で拒否し、管理APIは409を返す。
...
- **`authMiddleware`（Hono, Prisma `User.role` 参照）と `proxy.ts`（Supabase `user.app_metadata.role` 参照）はロールの参照元が別。** 両方を一致させる責務は「Prismaの `role` を変更した場所」にある: 招待経由の新規ユーザー作成（`lib/hono/routes/users.ts`）と管理者によるロール変更（`lib/hono/routes/admin.ts` の `patchUserRoute`）は、Prisma更新の直後に `createSupabaseAdminClient().auth.admin.updateUserById(supabaseId, { app_metadata: { role } })` で `app_metadata` も同期している。新しくロールを変更する経路を足すときはこの同期を忘れないこと（忘れるとAPIは通るのに `/admin` に入れない、またはその逆が起きる）。
- **service role key を使うAdmin APIクライアントは `lib/supabase/admin.ts` の `createSupabaseAdminClient()` に集約。** `@supabase/supabase-js` の `createClient` を各ファイルで直接呼ばない（セッション/クッキーを扱わない管理操作なので `@supabase/ssr` の `createServerClient` は不要——コードレビューでここを指摘されたことがあるが、cookieベースのセッションクライアントとservice roleのAdminクライアントは用途が別物）。
- **招待（`users.ts`）・パスワードリセット（`password-resets.ts`）のワンタイムトークンは「確認 → 使用 → 本処理」ではなく「`updateMany({ where: { token, usedAt: null, expiresAt: { gt: now } }, data: { usedAt: now } })` で先に原子的に確保 → `count !== 1` なら無効」の順で書く。** 検証と`usedAt`更新が別クエリだと同一トークンへの並行リクエストで二重使用できてしまう（招待なら複数アカウント作成、パスワードリセットなら二重更新）。招待は `prisma.$transaction` 内でuser作成までまとめて原子化し、パスワードリセットは外部API（Supabase Admin）呼び出し失敗時に `usedAt: null` へ戻して再試行可能にしている。新しいワンタイムトークン機能もこのパターンに合わせること。
